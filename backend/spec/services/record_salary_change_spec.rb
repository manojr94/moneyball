...
RSpec.describe RecordSalaryChange do
  ...
  describe 'success path' do
    ...
    it 'stores the correct attributes' do
      result = call(amount_minor_units: 8_500_00, currency: 'EUR', effective_date: Date.new(2024, 6, 1), reason: 'merit')
  ...
  describe 'validation failures' do
    it 'raises Error when the amount is zero' do
      expect { call(amount_minor_units: 0) }.to raise_error(RecordSalaryChange::Error, /greater than 0/)
    end

    it 'raises Error when the amount is negative' do
      expect { call(amount_minor_units: -1) }.to raise_error(RecordSalaryChange::Error)
    end

    it 'raises Error for an unknown currency' do
      expect { call(currency: 'XYZ') }.to raise_error(RecordSalaryChange::Error, /currency/i)
    end

    it 'raises Error when effective_date is missing' do
      expect { call(effective_date: nil) }.to raise_error(RecordSalaryChange::Error, /effective date/i)
    end

    it 'raises Error when reason is invalid' do
      expect { call(reason: 'bonus') }.to raise_error(RecordSalaryChange::Error, /reason/i)
    end

    it 'does not persist a salary row on validation failure' do
      expect { call(amount_minor_units: 0) rescue nil }.not_to change(Salary, :count)
    end
  end
end
