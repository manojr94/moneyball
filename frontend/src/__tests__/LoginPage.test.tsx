...
function renderLoginPage({ user = null as User | null, login = vi.fn() } = {}) {
  return render(
    <AuthContext.Provider value={makeCtx({ user, login })}>
...
describe('LoginPage', () => {
  ...
  it('redirects to /employees when already authenticated', () => {
    renderLoginPage({ user: { id: 1, name: 'Admin', role: 'hr_admin', email: 'a@a.com' } })
    expect(screen.getByText('Employees page')).toBeInTheDocument()
  })

  it('calls login with email and password on submit', async () => {
    const login = vi.fn().mockResolvedValue({ id: 1, name: 'Admin', role: 'hr_admin', email: 'a@a.com' })
