...
import type { PayGroup, CompaGroup, BandCoverageRow, PayAnalyticsResponse, CompaRatioResponse, BandCoverageResponse } from '../types'
...
const TAB_OPTIONS = ['pay', 'compa_ratio', 'band_coverage'] as const
...
function PayTable({ groups = [] }: { groups?: PayGroup[] }) {
...
function CompaTable({ groups = [] }: { groups?: CompaGroup[] }) {
...
function BandCoverageTable({ data }: { data?: BandCoverageResponse | BandCoverageRow[] | null }) {
  const uncovered = Array.isArray(data) ? data : (data as BandCoverageResponse | null)?.uncovered
...
export function AnalyticsDashboard() {
  ...
  const {
    ...
    loading: payLoading,
    error: payError,
  } = useApi<PayAnalyticsResponse>(payFetch, [groupBy, asOf, rateDate], { enabled: tab === 'pay' })
  ...
  const {
    ...
    error: compaError,
  } = useApi<CompaRatioResponse>(compaFetch, [groupBy, asOf, rateDate], { enabled: tab === 'compa_ratio' })
  ...
  const {
    ...
    error: coverageError,
  } = useApi<BandCoverageResponse>(coverageFetch, [], { enabled: tab === 'band_coverage' })
