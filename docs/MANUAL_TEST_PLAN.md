...
| M6.8 | Import a 10,000-row generated CSV (`bin/rails runner script/gen_import_csv.rb > /tmp/big.csv`). Time the request. Is it under, say, 30 seconds? Does it stay under 500 MB RSS? | Scale — the manual-test check for the M10 target, run once here to catch regressions early |
