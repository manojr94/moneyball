...
| **M11.5** | Cleanup & follow-ups | `GET /departments` endpoint + department filter dropdown on employee list; extract shared analytics grouping into `Analytics::GroupingSupport` concern (dedupes `PayAnalytics` / `CompaRatioAnalytics`); salary band create form (admin only); employee list sort extended to all columns (requires backend `SORT_COLUMNS` additions) with bidirectional ASC/DESC toggle (requires `sort_dir` param + keyset cursor redesign); employee list filter bar redesign — filter icon, panel supporting status + department + country + title/level | Filter-combination specs for all new filter params; band create form: admin sees it, viewer does not; existing analytics specs unchanged after refactor |
